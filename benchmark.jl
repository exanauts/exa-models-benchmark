...
if _BACKEND_ARG == "CUDA"
    using CUDA
    # depot precompiled without a driver bakes in "no CUDA runtime"; recompile CUDA_Runtime_jll and re-exec once (EXA_CUDA_HEALED bounds the retry)
...
# hardware.jl: DRIVER_INFO, VRAM_INFO, physical_cores, hardware_info, write_hw_toml.
...
function build_examodels_opf(filename; backend = nothing, form = :polar, T = Float64)
    # ExaModelsPower builds with prod = true internally; product callbacks work as-is.
...
function write_ampl_lv(name, model_func, args...)
    # cache key must include the case name; all LV cases share the same sizes
    nlfile = joinpath(AMPL_TMPDIR, "lv_$(name)_$(hash(args)).nl")
    if !isfile(nlfile)
        em = build_examodels_lv(model_func, args...)
        # write+rename so an interrupted write never leaves a truncated cache file
        tmp = nlfile * ".tmp"
        write_nl(tmp, em)
...
# Per-case wall-clock budget for the AMPL pipeline; slower cases are skipped.
const AMPL_TIMEOUT_SECONDS = parse(Float64, get(ENV, "AMPL_TIMEOUT_SECONDS", "300"))

# AMPL skip-list: exact (suite, problem, size) triples.
const AMPL_SKIP = Set{Tuple{String,String,Any}}([
])

# Deadline for body(); throws TimeoutException. The task is not stopped (Julia cannot interrupt C calls).
...
# Timing protocol (btime, DEVICE_SYNC, ...) is shared with compare/compare_ad.jl; do not fork it.
...
function benchmark_model(m; seconds = 0.5)
    ...
    # obj's result must be consumed or LLVM can delete the whole call.
...
# EXA_SHARD="i/N": run every N-th job starting from the i-th (1-based).
    ...
    for (i, (form, filename)) in enumerate(opf_jobs)
        let
            ...
            try
                ...
                m  = mk()
                tc = btime_create(mk; seconds = seconds)
                r  = benchmark_model(m; seconds = seconds)
        ...
        let
            ...
            try
                with_timeout(AMPL_TIMEOUT_SECONDS) do
                    ...
                    try
                        m  = read_ampl(nlfile)
                        tc = btime_create(() -> (am = read_ampl(nlfile); finalize(am)); seconds = seconds)
                        r  = benchmark_model(m; seconds = seconds)
