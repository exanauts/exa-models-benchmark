"""Coverage check for the Section 8.4 comparison CSVs.

`check_cases.jl` validates the Julia suites and knows nothing about these, so a
leg that dropped a framework entirely passed its coverage check and reported
success. That was not hypothetical: the CasADi OPF leg aborted on the first of
three cases and, under `set -e`, took the remaining ExaModels and JAX rows with
it. The invocations are now guarded so one framework cannot end a leg; this is
what makes the resulting absence loud instead of silent.

Fails only on a MISSING (problem, case/size, framework, device) combination that
the leg was supposed to produce. Usage:

    python check_compare.py <results-dir> --expect problem=opf --frameworks examodels,jax,torch --device cuda
"""
...
# A row identifies its problem explicitly; older files without the column are
# from a superseded schema and are ignored rather than guessed at.
