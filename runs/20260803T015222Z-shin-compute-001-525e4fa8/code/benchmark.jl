...
function write_ampl_lv(model_func, args...)
...
function write_ampl_cops(model_func, args...)
...
const AMPL_SKIP = Set{Tuple{String,String,Any}}([
    ("LV", "wood", 200000),  # .nl writer hangs / OOMs at 200k vars
])
...
function benchmark_model(m; seconds = 0.5)
    ...
    thprod = try btime(() -> hprod!(m, x, v, Hv); seconds = seconds) catch; NaN end
    tjprod  = ncon > 0 ? (try btime(() -> jprod!(m, x, v, Jv); seconds = seconds) catch; NaN end) : 0.0
    tjtprod = ncon > 0 ? (try btime(() -> jtprod!(m, x, y, Jtv); seconds = seconds) catch; NaN end) : 0.0
...
# Write a JuMP OPF model to a temporary .nl file for the AMPL (ASL) path.
function write_nl_opf(builder, ref)
    jm = builder(ref)
    nlfile = tempname() * ".nl"
    JuMP.write_to_file(jm, nlfile)
    return nlfile
end
...
function run_reference(; seconds = 0.5, suites = nothing)
        ...
        let
            ...
            try
                with_timeout(AMPL_TIMEOUT_SECONDS) do
                    nlfile = write_ampl_lv(case.model, args...)
                    m  = read_ampl(nlfile)
    ...
    for (i, (case, sz)) in enumerate(cops_jobs)
        let
            ...
            try
                with_timeout(AMPL_TIMEOUT_SECONDS) do
                    nlfile = write_ampl_cops(case.model, args...)
                    m  = read_ampl(nlfile)
                    tc = benchmark_creation(read_ampl, nlfile; seconds = seconds)
                    r  = benchmark_model(m; seconds = seconds)
    ...
    # OPF — AMPL (JuMP model written to .nl, read back through ASL)
    for (i, (form, filename)) in enumerate(opf_jobs)
        let
            builder = form == :polar ? build_jump_opf_polar : build_jump_opf_rect
            filepath = joinpath(pglib_dir(), filename)
            ...
            try
                with_timeout(AMPL_TIMEOUT_SECONDS) do
                    ref = parse_opf_data(filepath)
                    nlfile = write_nl_opf(builder, ref)
                    m  = read_ampl(nlfile)
...
function save_results(rows, hw_info, tag)
    ...
    # Write hardware info as TOML
    hw_toml = Dict(
        "hostname" => hw_info["hostname"],
        "device" => hw_info["device"],
        "precision" => get(hw_info, "precision", "fp64"),
