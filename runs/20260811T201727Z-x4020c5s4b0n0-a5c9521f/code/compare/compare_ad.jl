...
# Which problem. lv is Luksan-Vlcek 5.1 (banded); elec is COPS "electrons on a
...
if DEVICE in ("cuda", "amdgpu", "metal")
    error("device $DEVICE is not available in this checkout: CUDA.jl, AMDGPU.jl and " *
          "Metal.jl were removed from Project.toml so the environment resolves on an " *
          "Intel GPU machine. Use `oneapi`, `cpu` or `cpu-mt`.")
elseif DEVICE == "oneapi"
    using oneAPI
    oneAPI.functional() ||
        error("oneAPI is not functional on $(gethostname()) — no Level Zero GPU visible. " *
              "This leg must run inside a compute-node allocation, not on a login node.")
...
function build_model(backend, T)
    ...
    # one.
    PROBLEM == "elec" && return COPSBenchmark.elec_model(
        COPSBenchmark.ExaModelsBackend(), N; T = T, backend = backend)
    if PROBLEM == "opf"
...
elec's start point comes from Julia's RNG seeded at 2713, which numpy cannot
reproduce -- and a comparison where the frameworks sit at different points is
not a comparison. Exporting it from the model that defines the problem also
pins the variable ORDERING, which for elec is the x block, then y, then z.
"""
...
# For lv/elec the size IS N. For OPF, N is meaningless and the size that orders
