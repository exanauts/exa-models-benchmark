...
#
# This checkout is Intel-only: CUDA.jl, AMDGPU.jl and Metal.jl are not in the
# project, so CUDA/AMDGPU/Metal are rejected up front rather than dying inside
# `using` with a message that reads like a broken install.
#
...
if _BACKEND_ARG in ("CUDA", "AMDGPU", "Metal")
    error("$(_BACKEND_ARG) is not available in this checkout: CUDA.jl, AMDGPU.jl and " *
          "Metal.jl were removed from Project.toml so the environment resolves on an " *
          "Intel GPU machine. Use `oneAPI` (Intel), `CPU`, `nothing` or `reference`.")
elseif _BACKEND_ARG == "oneAPI"
    using oneAPI
    # oneAPI.jl carries its own compute runtime (NEO_jll) and Level Zero loader,
    # so it does not depend on a system oneAPI module being loaded. What it does
    # need is a GPU: a login node has no renderD node and reports zero devices,
    # which otherwise surfaces much later as an empty-collection error from the
    # first kernel launch.
    oneAPI.functional() ||
