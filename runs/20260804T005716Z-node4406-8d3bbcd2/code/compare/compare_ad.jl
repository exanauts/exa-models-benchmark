...
function run_examodels()
    Base.invokelatest() do
        ...
        DEVICE == "cpu" && @info "exported start point" file = export_problem(m)
...
out = joinpath("results", "compare_$(PROBLEM)_$(host)_$(FRAMEWORK)_$(DEVICE).csv")
row = DataFrame(problem = [PROBLEM], framework = [FRAMEWORK], device = [DEVICE],
                n = [N], threads = [Threads.nthreads()])
for k in CALLBACKS
