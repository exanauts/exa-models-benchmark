...
def _casadi_elec(ca, JIT):
    ...
    for k in CALLBACKS:
        ...
        for vname, opts in (("interp", None), ("jit", JIT)):
            F = ca.Function(f"e_{k}_{vname}", sym_in[k], [expr[k]], *([opts] if opts else []))
            cands.append((f"mx/{vname}", lambda F=F, a=num_in[k]: F(*a)))
        try:
            F = _codegen(ca, ca.Function(f"e_{k}_cg", sym_in[k], [expr[k]]), k)
            cands.append((f"mx/codegen", lambda F=F, a=num_in[k]: F(*a)))
        except Exception as e:
