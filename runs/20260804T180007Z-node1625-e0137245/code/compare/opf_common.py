...
class OPF:
    ...
    def _compare_sparse(name, got, want, ncol, tol):
        ...
        if gk.shape != wk.shape or not np.array_equal(gk, wk):
            ...
            where = ""
            if missing.size:
                r, c = divmod(int(missing[0]), ncol)
                where = f"; first missing entry ({r}, {c})"
            elif extra.size:
                r, c = divmod(int(extra[0]), ncol)
                where = f"; first extra entry ({r}, {c})"
            raise AssertionError(
                f"{name} pattern differs from ExaModelsPower: {missing.size} missing, "
                f"{extra.size} extra, of {wk.size} entries{where}")
