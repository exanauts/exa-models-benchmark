...
def _torch_opf(torch, tgrad, thessian, tvmap, dev):
    ...
    out = {}
    out["obj"] = select("obj", [("plain", W(obj))], o.ref["obj"], conv)
    out["cons"] = select("cons", [("plain", W(cons))], np.array(o.ref["cons"]), conv)
    out["grad"] = select("grad", [("plain", W(tgrad(obj)))], np.array(o.ref["grad"]), conv)
