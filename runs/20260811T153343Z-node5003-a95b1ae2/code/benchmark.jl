...
# Hardware identification lives in hardware.jl so a leg can record its machine
# without loading this file, which pulls in JuMP, PowerModels and
# NLPModelsJuMP. Defines DRIVER_INFO, VRAM_INFO, physical_cores,
# hardware_info and write_hw_toml.
...
# The timing protocol -- btime, btime_create, DEVICE_SYNC, BT_AUDIT, OBJ_SINK --
# lives in btime.jl and is shared with compare/compare_ad.jl. It used to live
# here, with a copy over there that had silently lost the GPU branch, so the
# section 8.4 tables were timed without synchronization while these ones were
# not. Include it; never copy it back.
