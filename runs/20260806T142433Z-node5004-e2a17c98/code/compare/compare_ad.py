...
def run_torch():
    ...
    # TWO second-order idioms, because the obvious one cannot be compiled.
    #
    # torch.func.hessian is jacfwd(jacrev(f)), and forward-mode AD under vmap
    # under torch.compile trips an internal PyTorch assert -- reproduced
    # interactively on node4111 with torch 2.13.0+cu130:
    #
    #   RuntimeError: InferenceMode::is_enabled() && self.is_inference()
    #   INTERNAL ASSERT FAILED at VariableMethodStubs.cpp:66 ... _fw_primal
    #
    # ("please report a bug to PyTorch" -- their assert, not our misuse.) It is
    # jacfwd specifically, not the Hessian and not vmap: every jacfwd-containing
    # composition fails under vmap and every reverse-only one compiles, while an
    # unvmapped hessian compiles fine.
    #
    #   vmap(hessian)         FAIL   |   vmap(jacrev(jacrev))  COMPILES
    #   vmap(jacfwd(jacrev))  FAIL   |   vmap(jacrev(grad))    COMPILES
    #   vmap(jacfwd(jacfwd))  FAIL   |   hessian, no vmap      COMPILES
    #
    # The cost of not knowing this was a PyTorch Hessian reported EAGER while its
    # own Jacobian was compiled -- 2.79 ms against 0.011 ms in the same row. That
    # is a harness artefact presented as a property of PyTorch's AD.
    #
    # Reverse-over-reverse computes the same Hessian (agreement 1.9e-16 at every
    # size) and BOTH are offered to select(), which measures and picks -- rather
    # than swapping one hard-coded idiom for another. Measured, because a
    # compilable-but-slower variant would be no improvement:
    #
    #   N        hessian eager   jacrev^2 eager   jacrev^2 compiled
    #   200           8.19 ms          1.84 ms           0.033 ms
    #   2000          3.92             2.10              0.097
    #   20000         6.81             4.91              0.723
    #   200000       47.41            38.82              9.11
    def _hess_builder(second_order, _name):
        hfk = tvmap(second_order(fk, 2))
        hck = tvmap(second_order(ck, 3))
        ...
        # A distinct code object per variant. types.FunctionType with a renamed
        # copy of the code gives each one its own identity for Dynamo's cache.
        code = hess_vmap.__code__.replace(co_name=_name)
        fresh = types.FunctionType(code, hess_vmap.__globals__, _name,
                                   hess_vmap.__defaults__, hess_vmap.__closure__)
        return fresh

    # Built through two SEPARATE calls that each define their own function, so
    # the variants do not share a Python code object. Dynamo caches compiled code
    # per code object: with both closures returned by one builder, a failed
    # compile of the first leaves an entry the second inherits, and the second
    # then reports "Guard failed on the same frame it was created on" -- which
    # reads as "this variant cannot compile either" and is purely an artefact of
    # attempt order. torch._dynamo.reset() in V() clears the cache, and keeping
    # the code objects distinct removes the interaction rather than papering over
    # it; both are cheap and the failure mode was expensive to find.
    _argn = lambda n: tuple(range(n))
    hess_vmap = _hess_builder(lambda f, n: thessian(f, argnums=_argn(n)),
                              "hess_fwd")
    hess_vmap_rr = _hess_builder(
        lambda f, n: tjacrev(tjacrev(f, argnums=_argn(n)), argnums=_argn(n)),
        "hess_rr")
    ...
    out["jac"] = select("jac", V(jac_vmap, "vmap"), r_jac, conv)
    out["hess"] = select("hess",
                          V(hess_vmap, "vmap") + V(hess_vmap_rr, "vmap-rr"),
                          r_hess, conv)
    return out, cpu_ratio(out["hess"][2])
